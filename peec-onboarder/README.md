...
The deep self-profile (stage 0b, see [`docs/INITIAL_DOMAIN_ENRICHMENT.md`](docs/INITIAL_DOMAIN_ENRICHMENT.md)) is what makes this gate work. Without it, the validator has nothing to compare against.
...
A real example output is committed at [`examples/snapshot_attio_2026-04-25.json`](examples/snapshot_attio_2026-04-25.json) — 462KB of structured GEO insights, fully populated, 100% model coverage.
...
| File | Purpose |
|---|---|
| `research/profile.py`      | Stage 0b: deep self-profile (Tavily ×9 + Anthropic synthesis → 15-field profile) |
| `research/discover.py`     | Stage 1+2: Tavily 3-approach discovery + consensus + Anthropic validation |
| `research/normalize.py`    | Stage 2: 5-step normalization (parent/child, canonical, dedupe, why backfill) |
| `research/anton_runner.py` | Stage 3+4: subprocess wrapper around Anton's TS prompt-gen pipeline |
| `../anton/scripts/prompts.ts` | Stage 3+4 entry: DataForSEO + Curator + Sub-agents + Aggregator |
| `research/push.py`         | Stage 5: Peec REST writes (wipe-and-replace) |
| `research/snapshot.py`     | Stage 7: 17 REST endpoints → unified snapshot |
| `research/mcp_client.py`   | Stage 7: MCP OAuth + `get_actions` + `get_project_profile` |
| `research/orchestrate.py`  | End-to-end CLI tying all stages together |
...
- [`docs/INITIAL_DOMAIN_ENRICHMENT.md`](docs/INITIAL_DOMAIN_ENRICHMENT.md) — the deep self-profile design (stage 0b) that grounds every downstream query
- [`examples/snapshot_attio_2026-04-25.json`](examples/snapshot_attio_2026-04-25.json) — full real snapshot for reference
